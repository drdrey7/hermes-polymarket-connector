...
from hermes_polymarket_action.geoblock import GeoblockResult
from hermes_polymarket_action.validation import ValidationResult
...
def mock_valid_market():
    """Context manager to mock validate_market_token to return valid."""
    return patch('hermes_polymarket_action.execution.validate_market_token', return_value=ValidationResult(
        valid=True,
...
def test_geoblock_blocked_blocks_execution(tmp_path):
    ...
    try:
        ...
        with mock_valid_market():
            with patch('hermes_polymarket_action.execution.check_geoblock') as mock_geo:
                mock_geo.return_value = GeoblockResult(
                    allowed=False,
                    country_code="RU",
                    error="Geoblock active: trading blocked in RU"
                )
    ...
    try:
        ...
        with mock_valid_market():
            with patch('hermes_polymarket_action.execution.check_geoblock') as mock_geo:
                mock_geo.return_value = GeoblockResult(
                    allowed=False,
                    error="Geoblock check failed: ConnectionError (blocked by default)"
                )
                plan = engine.build_execution_plan("ABCD1234", True)
                assert plan.allowed is False
...
def test_max_position_limit_blocks_execution(tmp_path):
    """Position exceeding MAX_POSITION_USD should be blocked when current_position_usd provided."""
    config = make_config(MAX_POSITION_USD="1000", MAX_ORDER_USD="1000", MAX_DAILY_USD="10000")
    # Use isolated daily_spent for risk engine
    from hermes_polymarket_action.risk import RiskEngine
    risk_engine = RiskEngine(config, daily_spent_path=tmp_path / "daily_spent3.json")
    ...
    try:
        with mock_valid_market():
            plan = engine.build_execution_plan("ABCD1234", True)
            assert plan.allowed is True  # 500 + 0 = 500 < 1000

            # Now test with current_position_usd set
            plan.order_request.current_position_usd = 600.0
            risk_allowed, risk_reason = engine.check_risk(plan.order_request)
            assert risk_allowed is False  # 600 + 500 = 1100 > 1000
            assert "exceed" in risk_reason.lower() or "position limit" in risk_reason.lower()
    finally:
        conf.audit_log = original
...
def test_max_open_orders_blocks_execution(tmp_path):
    """Open orders exceeding MAX_OPEN_ORDERS should be blocked."""
    config = make_config(MAX_OPEN_ORDERS="5", MAX_DAILY_USD="10000")
    audit = make_audit_log(tmp_path)
    config.audit_log_path = tmp_path / "audit.log"
    ...
    try:
        ...
        with mock_valid_market():
            # Mock client.get_orders to return 5 orders (at limit)
            mock_client = Mock()
            mock_client.get_orders.return_value = [{}, {}, {}, {}, {}]  # 5 orders
            engine._client = mock_client

            plan = engine.build_execution_plan("ABCD1234", True)
            assert plan.allowed is False
            assert "Open orders" in plan.reason
    finally:
        conf.audit_log = original
...
def test_audit_log_redacts_secrets(tmp_path):
    ...
    assert "0xaaaaaaaa" not in content
    assert "0xbbbbbbbb" not in content
...
def test_execution_engine_can_be_mocked(tmp_path):
    ...
    mock_client = Mock()
    mock_client.create_or_derive_api_creds.return_value = Mock(
        api_key="test", api_secret="test", api_passphrase="test"
    )
    mock_client.set_api_creds = Mock()
    engine._client = mock_client
...
def test_execute_order_calls_record_fill(tmp_path):
    """After successful execution, record_fill should be called to update daily spend."""
    config = make_config(MAX_DAILY_USD="10000")
    # Use isolated daily_spent for risk engine
    from hermes_polymarket_action.risk import RiskEngine
    risk_engine = RiskEngine(config, daily_spent_path=tmp_path / "daily_spent.json")
    engine = ExecutionEngine(config, risk_engine=risk_engine)
    ...
    import hermes_polymarket_action.audit as audit_mod
    import hermes_polymarket_action.confirmation as conf

    audit_log = AuditLog(tmp_path / "audit.log")
    audit_orig = audit_mod.audit_log
    conf_orig = conf.audit_log
    audit_mod.audit_log = audit_log
    conf.audit_log = audit_log

    try:
        audit_log.log(entry)

        mock_client = Mock()
        mock_client.create_and_post_order.return_value = {"order_id": "ORDER123", "tx_hash": "0x123"}
        mock_client.get_orders.return_value = []  # No open orders
        engine._client = mock_client
        ...
        with mock_valid_market():
            ...
            engine.risk_engine.record_fill = Mock()

            result = engine.execute_order(plan)

            assert result.status.value in ("FILLED", "SUBMITTED")
            engine.risk_engine.record_fill.assert_called_once_with(50.0)
    finally:
        audit_mod.audit_log = audit_orig
        conf.audit_log = conf_orig


def test_close_position_requires_confirmation_code(tmp_path):
    """close_position must require confirmation code and run gates."""
    config = make_config()
    audit = make_audit_log(tmp_path)
    config.audit_log_path = tmp_path / "audit.log"
    ...
    try:
        ...
        # Without confirmation code should fail
        result = engine.close_position("0x" + "1" * 64, 50.0, None, False)
        assert result.status.value == "REJECTED"
        assert "required" in result.error.lower() or "approval" in result.error.lower()

        # With confirmation code but no approval should fail
        result = engine.close_position("0x" + "1" * 64, 50.0, "ABCD1234", False)
        assert result.status.value == "REJECTED"
    finally:
        conf.audit_log = original
